...
import { UPDATE_PROFILE_DATA } from '../../redux/profile/constants'
...
const updateProfileMainDataSagaWorker = function* (action: ReturnType<typeof ProfileAC.updateProfileData>) {
    ...
    try {
        ...
        yield put(
            ErrorsAC.setErrOnUpdateProfileMainData(
                cutText(err.response?.data?.message || err.message, 30, 'Error on updating profile data'),
            ),
        )
    }
