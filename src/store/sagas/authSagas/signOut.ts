...
const signOutSagaWorker = function* () {
    ...
    try {
        const { resultCode, messages } = yield call(authFetchingAPI.offAuth)
        ...
        yield put(ErrorsAC.setErrOnSignOut(cutText(err.response?.data?.message || err.message, 30, 'Error on logging out')))
    }
