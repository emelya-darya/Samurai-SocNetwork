...
import { INCREASE_SCROLLED_MESSAGES_PAGE } from '../../redux/dialogs/constants'
...
const sagaWorkerIncreaseMessagesPortion = function* () {
    ...
    try {
        ...
        if (companionData) {
            const { items, error } = yield call(dialogsFetchingAPI.getMessagesPortion, companionData.id, currentPage + 1, portionSize)
            if (!error) {
        ...
        yield put(
            ErrorsAC.setErrOnLoadingExtraMessagesPortion(
                cutText(err.response?.data?.message || err.message, 50, 'Error on load messages portion'),
            ),
        )
    }
