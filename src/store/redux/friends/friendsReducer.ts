...
export const FriendsAC = {
    ...
    setActivePageFromQueryParams: (activePageFromQueryParams: number) =>
        ({ type: SET_ACTIVE_PAGE_FROM_QUERY_PARAMS, activePageFromQueryParams: activePageFromQueryParams } as const),

    setSearchRequestFromLC: (searchRequest: string) => ({ type: SET_SEARCH_REQUEST_FROM_LC, searchRequest: searchRequest } as const),
    setSearchRequestFromQueryParams: (searchRequest: string) =>
        ({ type: SET_SEARCH_REQUEST_FROM_QUERY_PARAMS, searchRequest: searchRequest } as const),
    ...
    changeFollowStatus: (friendId: number, currentFollowStat: boolean) =>
        ({ type: CHANGE_FOLLOW_STAT, friendId, currentFollowStat } as const),
    setNewFollowStat: (friendId: number) => ({ type: SET_NEW_FOLLOW_STAT, friendId } as const),
    setIsInProgressChangeFollowStat: (friendId: number, isInProgress: boolean) =>
        ({ type: SET_IS_IN_PROGRESS_CHANGE_FOLLOW_STAT, friendId, isInProgress } as const),
}
