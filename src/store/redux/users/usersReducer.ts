...
export const UsersAC = {
   ...
   setActivePageFromQueryParamsAC: (activePageFromQueryParams: number) => ({ type: SET_ACTIVE_PAGE_FROM_QUERY_PARAMS, activePageFromQueryParams } as const),
   ...
   changeFollowStatusAC: (userId: number, currentFollowStat: boolean) => ({ type: CHANGE_FOLLOW_STAT, userId, currentFollowStat } as const),
   setNewFollowStatAC: (userId: number) => ({ type: SET_NEW_FOLLOW_STAT, userId } as const),
   setIsInProgressChangeFollowStatAC: (userId: number, isInProgress: boolean) => ({ type: SET_IS_IN_PROGRESS_CHANGE_FOLLOW_STAT, userId, isInProgress } as const),
}
